...
def _inject_excessive_verify(cot: str, problem: Dict[str, str]) -> str:
    """Append 2-3 redundant Verify sentences."""
    templates = random.sample(VERIFY_TEMPLATES, k=random.randint(2, 3))
    injected = " ".join(t.format(expr=problem["a"], result=problem["a"], var="x") for t in templates)
    return f"{cot} {injected}"


def _inject_dead_branch(cot: str, problem: Dict[str, str]) -> str:
    """Insert a dead-branch sentence in the middle."""
    wrong = str(random.randint(1, 100))
    tmpl = random.choice(DEAD_BRANCH_TEMPLATES)
...
def _inject_hedge_backtrack(cot: str) -> str:
    """Insert a hedged backtrack sentence."""
    tmpl = random.choice(HEDGE_BACKTRACK_TEMPLATES)
    parts = cot.split(" ")
    mid = len(parts) // 2
...
def _inject_long_transform_chain(cot: str) -> str:
...
def _inject_circular_retrieval(cot: str) -> str:
    """Insert the same theorem/recall 2-3 times."""
    tmpl = random.choice(CIRCULAR_RETRIEVAL_TEMPLATES)
    repeats = " ".join([tmpl] * random.randint(2, 3))
...
def generate_synthetic_cots(n: int = 50, seed: int = 42) -> List[Dict[str, str]]:
    ...
    for i in range(n):
        ...
        selected = random.sample(redundancy_injectors, k=k)
        types = []
        cot = clean
        for name, injector in selected:
            types.append(name)
            if name in ("DeadBranch",):
                cot = injector(cot, base)
            elif name in ("ExcessiveVerify",):
                cot = injector(cot, base)
            else:
                cot = injector(cot)
